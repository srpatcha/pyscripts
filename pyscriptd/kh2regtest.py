...
# Rick Hale 9/14/2016: modified to run under python 2.4.3 and parse ssh_host_rsa_key.pub
...
def convertStr(x):
    """ rick hale: converted lambda expression to function for python 2.4.3 compatability """
...
def get_dms_ipaddr():
    ip = None
    try:
        # if this is a vagrant dms, it'll use eth1
        ip = get_ip_address('eth1')  # '192.168.0.110'
    except IOError:
        # standard DMS is on eth0
...
for line in fileinput.input(args):
    ...
    try:
        ...
        if re.match (r"\d*$", fields[1]):
            ...
            # Rick Hale: modified to parse ssh_host_rsa_key.pub
            sshkeytype, blob = fields[0], base64.decodestring (fields[1])
        ...
        for host in string.split (hostpat, ','):
            if re.search (r"[*?!]", host):
                ...
                if m:
                    ...
                    # rick hale -- add something that would be easy to search/replace
                    host = get_dms_ipaddr()
                    port = 22
                ...
                # value = string.join (map (
                    ...
                    # keyparams), ',')
                # rick hale -- replace lambda expression -- caused problems in python 2.4.3...
                value = string.join (map (convertStr, keyparams), ',')
                if output_type == 'unix':
                    # Unix format.
